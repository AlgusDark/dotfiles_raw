...
local user_icon = hs.styledtext.new('  ', { font = {name = 'JetBrainsMono Nerd Font', size = 12 }, color = {hex = '#8e8e8e'}})
local ticket_icon = hs.styledtext.new('  ', { font = {name = 'JetBrainsMono Nerd Font', size = 12 }, color = {hex = '#8e8e8e'}})
...
local function updateMenu()
    ...
    hs.http.asyncGet(jira_url, {Authorization = auth_header}, function(status, body)
        ...
        obj.indicator:setTitle(numberToSuperscript(#issues))
                ...
                if status ~= 200 then
                    ...
                    for _, transition in ipairs(transitions) do
                        ...
                        table.insert(transitions_submenu, {
                            ...
                            checked= transition.to.id == issue.fields.status.id,
                            -- image = hs.image.imageFromName('NSTouchBarTagIconTemplate'),
