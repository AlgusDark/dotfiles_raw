...
function openURL(url_patterns, default_handler, url)
   for i,pair in ipairs(url_patterns) do
      local p = pair[1]
      local app = pair[2]
      local func = pair[3]
      obj.logger.df("  Testing URL with pattern '%s'", p)
      if string.match(url, p) then
         id = app
         if id ~= nil then
            obj.logger.df("    Match found, opening with '%s'", id)
            hs.application.launchOrFocusByBundleID(id)
            hs.urlevent.openURLWithBundle(url, id)
            return
         end
         if func ~= nil then
            obj.logger.df("    Match found, calling func '%s'", func)
            func(url)
            return
         end
      end
   end
   obj.logger.df("No match found, opening with default handler '%s'", default_handler)
   hs.application.launchOrFocusByBundleID(default_handler)
   hs.urlevent.openURLWithBundle(url, default_handler)
end
...
function obj:dispatchURL(scheme, host, params, fullUrl, senderPID)
   local url = fullUrl
   local currentApp =(hs.application.applicationForPID(senderPID) or hs.application.frontmostApplication()):name()
   ...
   --
   if self.perAppConfig[currentApp] then
      local appConfig = self.perAppConfig[currentApp]
      openURL(appConfig.url_patterns or {}, appConfig.default_handler or self.default_handler, url)
   else
      openURL(self.url_patterns, self.default_handler, url)
   end
   --
